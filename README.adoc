= GitHub Action: Manage artifact version labels in the CloudBees platform

Use this GitHub Action (GHA) to create, update, and remove labels on existing artifact versions in the CloudBees platform.
...
----
        ...
        - name: Revise artifact labelling
          ...
          with:
            artifact-id: "1234abcd-56ef-78ab-90cd-efabcd123456"
...
The following GHA workflow example uses this action to register an artifact version and then update its labels in the CloudBees platform.
